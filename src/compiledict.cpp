...
#define N_LETTERGP_ITEMS 200
	SAVESTACK char *items[N_LETTERGP_ITEMS];
	SAVESTACK char item_length[N_LETTERGP_ITEMS];
...
{//====================================================================
	...
	int compile_mode=0;
	char *buf;
	SAVESTACK char buf1[500];
	SAVESTACK char *rules[N_RULES];

	int n_rgroups = 0;
	int n_groups3 = 0;
	SAVESTACK RGROUP rgroup[N_RULE_GROUP2];
...
// flags: bit 0:  include source line number information, for debug purposes.
	...
	int offset_rules=0;
	int value;
	SAVESTACK char fname_in[sizeof(path_home)+45];
	SAVESTACK char fname_out[sizeof(path_home)+15];
	SAVESTACK char fname_temp[sizeof(path_home)+15];
	SAVESTACK char path[sizeof(path_home)+40];       // path_dsource+20
