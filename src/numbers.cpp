...
// "words" of 3 digits may be preceded by another number "word" for thousands or millions
	...
	int len;
	char *p;
	SAVESTACK char string[32];  // for looking up entries in **_list
	SAVESTACK char buf1[100];
	SAVESTACK char ph_append[50];
	SAVESTACK char ph_buf[200];
	SAVESTACK char ph_buf2[50];
	SAVESTACK char ph_zeros[50];
	SAVESTACK char suffix[30];  // string[] must be long enough for sizeof(suffix)+2
	SAVESTACK char buf_digit_lookup[50];
