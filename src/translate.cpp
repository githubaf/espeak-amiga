...
// word1 is terminated by space (0x20) character
	...
	int prefix_stress;
	char *wordx;
	SAVESTACK char phonemes[N_WORD_PHONEMES];
	SAVESTACK char phonemes2[N_WORD_PHONEMES];
	SAVESTACK char prefix_phonemes[N_WORD_PHONEMES];
	SAVESTACK char unpron_phonemes[N_WORD_PHONEMES];
	SAVESTACK char end_phonemes[N_WORD_PHONEMES];
	SAVESTACK char end_phonemes2[N_WORD_PHONEMES];
	SAVESTACK char word_copy[N_WORD_BYTES];
	SAVESTACK char word_copy2[N_WORD_BYTES];
...
{//=================================================================================================
	...
	int word_flags;
	int word_copy_len;
	SAVESTACK char word_copy[N_WORD_BYTES+1];
	SAVESTACK char word_replaced[N_WORD_BYTES+1];
	SAVESTACK char old_dictionary_name[40];
...
{//==========================================================================================================
	...
	SAVESTACK short charix[N_TR_SOURCE+4];
	SAVESTACK WORD_TAB words[N_CLAUSE_WORDS];
	static char voice_change_name[40];
	int word_count=0;      // index into words

	SAVESTACK char sbuf[N_TR_SOURCE];
...
// Brackets are now recognised in TranslateRules()
	...
	for(ix=0; ix < word_count; ix++)
	{
		...
		char *pw;
		int nw;
		SAVESTACK char number_buf[150];
		SAVESTACK WORD_TAB num_wtab[50];  // copy of 'words', when splitting numbers into parts
