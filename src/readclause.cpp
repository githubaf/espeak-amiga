...
const wchar_t* wcschr (const wchar_t* str, wchar_t c)  //AF
{
   while(*str != 0)
   {
      if(*str == c)
...
size_t wcslen(const wchar_t *str) //AF
{
	int ix=0;
...
double wcstod (const wchar_t* str, wchar_t** tailptr)   //AF
{
   int ix;
   char buf[80];
...
{//===================================================
	...
	int *ip;
	int  length;
	SAVESTACK char fname_temp[100];
	SAVESTACK char fname2[sizeof(path_home)+13+40];
	...
	if((f = fopen(fname,"rb")) != NULL)
	{
		...
		int fd_temp;
		int header[3];
		SAVESTACK char command[sizeof(fname2)+sizeof(fname2)+40];
...
*/
	...
	SAVESTACK wchar_t xml_buf[N_XML_BUF+1];
