		...
		default:
			...
			*outptr++ = (char)(max_ph);
			...
			if(max_ph == phonSWITCH)
			{
				...
				char *p_lang = outptr;
				while( c = *p, c != '\0' && !isspace(c)) /*   while(!isspace (c = *p) && (c != 0)) */ /* sonst kommt ANSI C++ forbids braced-groups within expressions */	
				{
					p++;
					*outptr++ = tolower(c);
				}
...
*/
	...
	SAVESTACK signed char vowel_stress[N_WORD_PHONEMES/2];
	SAVESTACK char syllable_weight[N_WORD_PHONEMES/2];
	SAVESTACK char vowel_length[N_WORD_PHONEMES/2];
	SAVESTACK unsigned char phonetic[N_WORD_PHONEMES];
