...
{//=========================
	int finished;
	SAVESTACK unsigned char wav_outbuf[1024];
	SAVESTACK char fname[210];
...
{//=====================================================
	...
	home=getenv("HOME");
    snprintf(path_home,sizeof(path_home),"%s/espeak-data", home ? home:"");  /* avoid potential Null-Ptr access */
...
/* for atexit()
 * stack must be filled with pattern at the beginning of main()
 */
// define via command line if needed
#ifdef SHOW_AMIGA_STACK
void ShowTotalSTackUsage(void)
{
    static ULONG *StackFillPtr;
 	struct Task *TaskPtr = FindTask(NULL);

	StackFillPtr=(ULONG*)TaskPtr->tc_SPLower;

    while(StackFillPtr < TaskPtr->tc_SPUpper && *StackFillPtr==0x5374636b) { StackFillPtr++;} // look for Stack still filled with pattern "Stck"

    printf("Max Stack Nutzung:     %8lu Bytes\n",(ULONG)TaskPtr->tc_SPUpper-(ULONG)StackFillPtr);
...
//		{"brief",   no_argument,       &verbose_flag, 0},
	...
	espeak_VOICE voice_select;
	SAVESTACK char filename[200];
	SAVESTACK char voicename[40];
...
// define via command line if needed
    ...
    static void *StackPtr;
    static ULONG *StackFillPtr;
 	struct Task *TaskPtr = FindTask(NULL);

    __asm__ ( "move.l a7,%0" : "=&a" (StackPtr) );

    StackFillPtr=(ULONG*)TaskPtr->tc_SPLower;

    while(StackFillPtr<StackPtr) {*StackFillPtr++ = 0x5374636b;} // Fill available stack with pattern "Stck"

//    printf("stack pointer: $%08lx\n",(ULONG)StackPtr);
//	printf("tc_SPLower:    $%08lx\n",(ULONG)TaskPtr->tc_SPLower); /* stack lower bound    */
//	printf("tc_SPUpper:    $%08lx\n",(ULONG)TaskPtr->tc_SPUpper); /* stack upper bound +2 */
	printf("Stack:                 %8lu Bytes\n",(ULONG)TaskPtr->tc_SPUpper - (ULONG)TaskPtr->tc_SPLower );
//	printf("Stack noch verfuegbar: %8lu Bytes\n", (ULONG)StackPtr - (ULONG)TaskPtr->tc_SPLower);
//	printf("Stack verbraucht:      %8lu Bytes\n", (ULONG)TaskPtr->tc_SPUpper - (ULONG)StackPtr);

	atexit(ShowTotalSTackUsage);
